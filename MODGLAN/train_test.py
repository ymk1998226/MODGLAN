...
from layers import PositionalEncoding, LearnedPositionalEncoding, MultiClassFocalLossWithAlpha
...
def train_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict, theta_smooth, theta_degree, theta_sparsity, neta, train_VCDN=True):
    loss_dict = {}
    for m in model_dict:
        model_dict[m].train()

    # criterion = MultiClassFocalLossWithAlpha(reduction='mean')
    ...
    for i in range(num_view):
        ...
        d_adj = adj_list[i]
        d_adj[d_adj != 0] = 1
        normalized_adj = normalize_adj(final_adj,d_adj)
        ...
        weight = torch.norm(WF_weight[i], p=2, dim=1)
        weight_norm.append(weight)

        tol_loss = ci_loss + graph_loss + 0.0001 * WF_L12_loss
        tol_loss_list.append(tol_loss)
    ...
    if train_VCDN and num_view >= 2:
        ...
        atten_data_list = model_dict["A"](GCN_list)

        #Position Embedding
        atten_data_list = PositionalEncoding(num_hiddens=atten_data_list[0].size(1), dropout=0.5, num_modals=atten_data_list.size(0))(
            atten_data_list.transpose(1, 0) * math.sqrt(atten_data_list[0].size(1))).transpose(1, 0)

        #Learnable Position Embedding
        # atten_data_list = LearnedPositionalEncoding(num_hiddens=atten_data_list[0].size(1), dropout=0.5, num_modals=3)(
        #     atten_data_list.transpose(1, 0) * math.sqrt(atten_data_list[0].size(1))).transpose(1, 0)
...
def test_epoch(data_list, adj_list, model_dict, neta):
    ...
    atten_data_list = model_dict["A"](ci_list)
    new_data = torch.cat([atten_data_list[0], atten_data_list[1], atten_data_list[2]], dim=1)
    if num_view >= 2:
        c = model_dict["C"](new_data)
    else:
...
def train_test(data_folder, view_list, num_class,
    ...
    if data_folder == './BRCA_split/BRCA1':
        adj_parameter = 8
    ...
    if data_folder == './KIPAN_split/KIPAN1':
        adj_parameter = 8
        mode = 'weighted-cosine'
        # mode = 'adaptive-learning'
        featuresSelect_list = [1200, 1200, 1200]
        dim_he_list = [1200, 1200]
        input_data_dim = [dim_he_list[-1], dim_he_list[-1], dim_he_list[-1]]

    if data_folder == './SCC_split/SCC1':
        adj_parameter = 8
        mode = 'weighted-cosine'
        # mode = 'adaptive-learning'
        featuresSelect_list = [800, 800, 800]
        dim_he_list = [800, 800]
        input_data_dim = [dim_he_list[-1], dim_he_list[-1], dim_he_list[-1]]
    ...
    for epoch in range(num_epoch_pretrain):
        train_epoch(data_tr_list, adj_tr_list, labels_tr_tensor,
                    onehot_labels_tr_tensor, sample_weight_tr, model_dict, optim_dict, theta_smooth, theta_degree, theta_sparsity, neta, train_VCDN=False)

    print("\nTraining...")
    optim_dict = init_optim(num_view, model_dict, lr_e, lr_c,reg)
    for epoch in range(num_epoch+1):
        _, weight = train_epoch(data_tr_list, adj_tr_list, labels_tr_tensor,
        ...
        if epoch == num_epoch:
            _, idx1 = torch.sort(weight[0], descending=True)
            _, idx2 = torch.sort(weight[1], descending=True)
            _, idx3 = torch.sort(weight[2], descending=True)
            print('weight1:', idx1[:20])
            print('weight2:', idx2[:20])
            print('weight3:', idx3[:20])