...
def cal_adj_mat_parameter(edge_per_node, data, metric="cosine"):
    ...
    dist2 = torch.sort(dist.reshape(-1, ))
...
def show_heatmaps(matrices, xlabel, ylabel, figsize, cmap='Reds'):
    use_svg_display()
    ...
    for i, (row_axes, row_matrices) in enumerate(zip(axes, matrices)):
        for j, (ax, matrix) in enumerate(zip(row_axes, row_matrices)):
            ...
            pcm = ax.imshow(matrix.detach().numpy(), cmap=cmap, aspect='auto', origin='lower',vmin=-1,vmax=1)
            if i == num_rows - 1:
