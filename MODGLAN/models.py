...
class TransformerEncoder(nn.Module):
    def __init__(self, input_data_dims, hyperpm, num_class):
        ...
        self.Outputlayer = OutputLayer(d_in, self.d_v * self.n_head, 96, self.n_class, self.modal_num, self.dropout)

    def forward(self, x):
        bs = x.size(0)
        attn_map = []
        res_map = []
        x, _attn, residual = self.InputLayer(x)
        attn = _attn.mean(dim=1)
        attn_map.append(attn.detach().cpu().numpy())

        for i in range(self.n_layer):
            x, _attn, residual = self.Encoder[i](q=x, k=x, v=x, modal_num=self.modal_num)
            res_map.append(residual)
            attn = _attn.mean(dim=1)
        ...
        x = x.view(bs, -1)
        attn = attn.view(bs, -1)
        residual = residual.view(bs, -1)
        output = self.Outputlayer(x, residual, attn)
        return output
...
def init_model_dict(num_view, num_class, dim_list, dim_he_list, input_data_dims, hyperpm, adj_parameter, mode,featuresSelect_list,gcn_dopout=0.5):
    ...
    if num_view >= 2:
        model_dict["A"] = Attention_mechanism()
...
def init_optim(num_view, model_dict, lr_e, lr_c,reg):
    ...
    if num_view >= 2:
        optim_dict["C"] = torch.optim.Adam(list(model_dict["A"].parameters()) + list(model_dict["C"].parameters()), lr=lr_c,weight_decay=reg)
    return optim_dict